...
while True:
              ...
              if event.key == K_RIGHT:
                 ...
                 message="Level  Cleared"
                  ...
                  for area in level:
                      ...
                      if len(area.tile)>0:
                         top_left_limit=min(area.tile,key=lambda x:x.pos).pos
                         bottom_right_limit=max(area.tile,key=lambda x:x.pos).pos
                         for i in range(len(area.tile)):
                             if top_left_limit==area.tile[i].pos:
                                area.top_left_limit=i
                             elif bottom_right_limit==area.tile[i].pos:
                                area.bottom_right_limit=i
                      for i in range(len(area.tile)):
                          if area.tile[i].type=="solid":
                             area.solid_tile_index.append(i)
                  with open('level_3.lvl', 'wb') as file:
    ...
    if screen_mode=="edit" and edit_mode=="tile": 
       #if event.button == 1:
       if buttons[0]:
          for tile2 in selected_tiles:
              selected_cell[0]=pointed_cell[0]-moved_distance2_x+tile2[2]
              selected_cell[1]=pointed_cell[1]-moved_distance2_y+tile2[3]
              #selected_cell=pointed_cell
              contains=False
              for tile in current_area.tile:
                  if [selected_cell[0],selected_cell[1]]==tile.pos:
                     contains=True
                     break
              if not contains:
                 tile=Tile([tile2[0],tile2[1],tile_width,tile_height],
                 [selected_cell[0],selected_cell[1]],tile_type,tileset_index)
                 #level_tile_index.append(selected_tile_index)
                 if tile_type=="gate":
                    tile.connected_area=connected_area
                    tile.connected_area_pos=connected_area_pos
                    tile.area_moved_dist=moved_dist
                 current_area.tile.append(tile)
          ...
          for i in range(len(current_area.tile)):
              if [selected_cell[0],selected_cell[1]]==current_area.tile[i].pos:
    ...
    if screen_mode=="select":
       ...
       for i in range(len(current_area.tile)):
           pos_x=current_area.tile[i].pos[0]+moved_distance2_x
           pos_y=current_area.tile[i].pos[1]+moved_distance2_y
           if pos_x>=0 and pos_x<SCREEN_WIDTH and pos_y>=0 and pos_y<SCREEN_HEIGHT:
              screen.blit(tileset[current_area.tile[i].tileset_index],(pos_x,pos_y),
              current_area.tile[i].image_offset)
              if current_area.tile[i].type=="solid" and not hide_bounding_box:
                 pygame.draw.rect(screen, (255,200,0), [pos_x, pos_y, tile_width, tile_height], 4)
              elif current_area.tile[i].type=="gate" and not hide_bounding_box:
